...
class HomepageController extends Controller
{
    ...
    {
        return view('admin.homepage.index', ['sections' => $this->homepageService->paginate()]);
    }

    public function create()
    {
        return view('admin.homepage.create', ['products' => Product::ordered()->get()]);
    }

    public function store(StoreHomepageSectionRequest $request)
    {
        $data = $request->validated();
        $this->homepageService->create($data, $data['product_ids'] ?? []);

        return redirect()->route('admin.homepage.index')->with('success', 'Section created.');
    }

    public function edit(HomepageSection $homepageSection)
    {
        return view('admin.homepage.edit', [
            'section' => $this->homepageService->find($homepageSection->id),
    ...
    {
        $data = $request->validated();
        $this->homepageService->update($homepageSection, $data, $data['product_ids'] ?? []);
    ...
    {
        $this->homepageService->delete($homepageSection);

        return redirect()->route('admin.homepage.index')->with('success', 'Section deleted.');
    }
}
