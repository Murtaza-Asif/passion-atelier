...
class Product extends Model {
    ...
    protected $fillable = [
        ...
        'title', 'slug', 'sku', 'barcode', 'short_description', 'full_description',
    ...
    public function faqs(): HasMany { return $this->hasMany(ProductFaq::class); }
    ...
    public function getFeaturedImageUrlAttribute(): ?string { return $this->featured_image ? asset('storage/'.$this->featured_image) : null; }
    public function getOgImageUrlAttribute(): ?string { return $this->og_image ? asset('storage/'.$this->og_image) : null; }
    ...
    public function getImageKeyAttribute(): ?string
    {
        ...
        $s = $this->slug;
        if (str_contains($s, 'cotton')) return 'cotton';
        if (str_contains($s, 'wash') || str_contains($s, 'wear')) return 'washwear';
        if (str_contains($s, 'latha')) return 'latha';
        return 'custom';
    }
    ...
    {
        $s = $this->slug;
        if (str_contains($s, 'cotton')) return 'All-season';
        if (str_contains($s, 'wash') || str_contains($s, 'wear')) return 'Spring · Autumn · Winter';
        if (str_contains($s, 'latha')) return 'Year-round formal';
        return 'All-season';
    }
}
