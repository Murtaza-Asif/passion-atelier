...
class OfferService {
    ...
    public function find(int $id): Offer { return Offer::with('products', 'getProduct')->findOrFail($id); }
    public function create(array $data, array $productIds = []): Offer {
        $offer = Offer::create($data);
        if (!empty($productIds)) $offer->products()->sync($productIds);
        return $offer->fresh();
    }
    ...
    public function delete(Offer $offer): void { $offer->delete(); }
}
