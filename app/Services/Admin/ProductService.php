...
class ProductService {
    ...
    public function create(array $data, ?UploadedFile $featuredImage = null, ?UploadedFile $ogImage = null, array $gallery = []): Product {
        if ($featuredImage) $data['featured_image'] = $featuredImage->store('products', 'public');
        if ($ogImage) $data['og_image'] = $ogImage->store('products/seo', 'public');
        if (!empty($gallery)) {
            $paths = [];
            foreach ($gallery as $file) { $paths[] = $file->store('products/gallery', 'public'); }
    ...
    public function duplicate(Product $product): Product {
        ...
        $clone->slug = $product->slug . '-copy';
        $clone->sku = $product->sku ? $product->sku . '-copy' : null;
        $clone->title = $product->title . ' (Copy)';
        $clone->save();
        foreach ($product->variants as $v) { $clone->variants()->create($v->replicate()->toArray()); }
        foreach ($product->tags as $t) { $clone->tags()->attach($t->id); }
        return $clone->fresh()->load(['productType', 'category', 'brand', 'collection', 'variants', 'tags']);
    }
    public function syncTags(Product $product, array $tagIds): void { $product->tags()->sync($tagIds); }
