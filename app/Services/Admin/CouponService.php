...
class CouponService {
    ...
    public function find(int $id): Coupon { return Coupon::with('products', 'categories')->findOrFail($id); }
    public function create(array $data, array $productIds = [], array $categoryIds = []): Coupon {
        $coupon = Coupon::create($data);
        if (!empty($productIds)) $coupon->products()->sync($productIds);
        if (!empty($categoryIds)) $coupon->categories()->sync($categoryIds);
        return $coupon->fresh();
    }
    ...
    public function delete(Coupon $coupon): void { $coupon->delete(); }
}
